...
export function monthRate(ratePerHour, discount) {
  ...
  let valorPorMes = dayRate(ratePerHour)* 22;
  let precoFinal = applyDiscount(valorPorMes,discount)
...
export function daysInBudget(budget, ratePerHour, discount) {
  ...
  let valorPorMes =dayRate(ratePerHour);
  let valorParcial = applyDiscount(valorPorMes,discount) 
  return Math.floor(budget/valorParcial);
